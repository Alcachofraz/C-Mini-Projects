...
int compareSearch(const void * key, const void * w) {
	return strcmp((char*)key, (char*)(((Word*)w)->word));
}

int compareSort(const void * a, const void * b) {
	return strcmp((char*)(((Word*)a)->word), (char*)(((Word*)b)->word));
}
...
Word *indexFindWord(char *data) {
	return (Word*)bsearch(data, i->words, i->used, sizeof(Word), compareSearch);
}
...
void indexAddWord(char *data, int fileIdx, int line, long offset) {
	...
	qsort(i->words, i->used, sizeof(Word), compareSort);
}
